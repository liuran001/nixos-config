...
let
  ...
  pjsk-cursor-theme = pkgs.stdenvNoCC.mkDerivation {
    pname = "pjsk-cursor-theme";
    version = "0.0.5";

    # 所有 zip 作为固定输出源；hash 已固定，构建全程无需联网。
    srcs = map (z: pkgs.fetchurl { inherit (z) name url hash; }) cursorZips;
    dontUnpack = true;

    nativeBuildInputs = [
      pkgs.unzip
      pkgs.win2xcur # 把 Windows .ani/.cur 指针转成 X11 光标格式
    ];
...
in
{
  # 安装到系统 profile 后，KDE 光标设置即可列出全部 PJSK 主题。
  environment.systemPackages = [ pjsk-cursor-theme ];
}
